...
func TestViewer_Selected_ReturnsPointerToSliceElement(t *testing.T) {
	v := New(Options{Renderer: staticRenderer{lines: []string{"x"}}})
	v.elements = []NavElement{{Type: NavElementURL, Text: "before"}}
	v.selectedIndex = 0
	...
	sel.Text = "after"
	if v.elements[0].Text != "after" {
		t.Fatalf("expected element to be updated via Selected() pointer; got %q", v.elements[0].Text)
	}
}
