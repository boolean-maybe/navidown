...
// Selected returns the currently selected element, or nil if none.
func (v *Viewer) Selected() *NavElement {
	if v.selectedIndex < 0 || v.selectedIndex >= len(v.elements) {
		return nil
	}
	return &v.elements[v.selectedIndex]
}
...
func (v *Viewer) SetMarkdownWithSource(content string, sourceFilePath string, pushToHistory bool) error {
	...
	v.elements = v.parseMarkdownWithSource([]byte(content), sourceFilePath)
...
func (v *Viewer) correlatePositions() {
	...
	for i := range v.elements {
		...
		if found {
			correlations[i] = correlation{found: true, elemIdx: i, lineIdx: lineIdx, startCol: startCol, endCol: endCol}
		}
	}
	...
	for i := range correlations {
		...
		key := fmt.Sprintf("%d:%d", c.lineIdx, c.startCol)
		matchLen := c.endCol - c.startCol
		existingLen, exists := usedPositions[key]
		if !exists || matchLen > existingLen {
			usedPositions[key] = matchLen
		}
	}
