...
func (mc *MarkerCorrelator) isCacheValid(renderedLines []string) bool {
	if mc.cachedPositions == nil || len(mc.cachedLines) != len(renderedLines) {
		return false
	}
	// Simple pointer comparison - if lines slice is same, cache is valid
	if len(renderedLines) > 0 && len(mc.cachedLines) > 0 {
		return &renderedLines[0] == &mc.cachedLines[0]
	}
	return true
}
